class ServerPanelBase {
	...
	void SyncPlayersRequest( CallType type, ref ParamsReadContext ctx, ref PlayerIdentity sender, ref Object target ) {
		...
		if ( type == CallType.Server && GetGame().IsServer() ) {
			...
			GetRPCManager().SendRPC( "ServerPanelI", "SyncPlayers", new Param5<ref TStringArray, ref TIntArray, ref TFloatArray, string, bool> (playerListS, plyData, plyFData, Item, m_HasDisease), true, sender );
