class ServerPanelMenu extends UIScriptedMenu {
	...
	override void OnShow()	{
		...
		GetGame().GetMission().PlayerControlDisable(true);
	...
	private void SPGetPosition()	{
		if(GetGame().IsClient() || !GetGame().IsMultiplayer())	{
			if (Player){
				vector plyPos = Player.GetPosition();
				m_TextPlayerPos.SetText("Pos. X: " + Math.Floor(plyPos[0]).ToString() + "  Y: " + Math.Floor(plyPos[2]).ToString());
			}
		}
	}
		...
		if(GetGame().IsClient() || !GetGame().IsMultiplayer())	{
			...
			for (int att = 0; att < attcount; att++)	{	
				...
				if ( attachment.IsItemBase() )	{
					...
					total_load += attachmentIB.GetPlayerLoad();
				}
			}
	...
	void SyncPlayers( CallType type, ref ParamsReadContext ctx, ref PlayerIdentity sender, ref Object target ) {
		Param5<ref TStringArray, ref TIntArray, TFloatArray, string, bool> syncDataS;
		ref TStringArray PlayerListC		= new TStringArray;
		...
		if ( type == CallType.Client && GetGame().IsClient() || !GetGame().IsMultiplayer() ) {
			...
			string sItemHands	= syncDataS.param4;
			bool sDisease		= syncDataS.param5;
